...
describe "CronParser#next" do
  [
    ...
    ["0 0 * * W#5",           "2022-01-31 00:00",  "2022-02-07 00:00"],
    ["0 0 * 4 W#5",           "2022-01-31 00:00",  "2022-04-07 00:00"],
...
describe "CronParser#last" do
  [
    ...
    ["0 0 * * W#5",           "2022-01-31 00:00",  "2022-01-07 00:00"],
    ["0 0 * 4 W#5",           "2022-01-31 00:00",  "2021-04-07 00:00"],
  ].each do |line, now, expected_next|
    it "should return #{expected_next} for '#{line}' when now is #{now}" do
      now = parse_date(now)
      expected_next = parse_date(expected_next)
